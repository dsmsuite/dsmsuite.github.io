...
Sub Banding_Unpartitioned_DSM()  'finds the most upstream set of activities that can be worked concurrently in the time step
 
 Application.ScreenUpdating = False
       
    ReDim DSM(elementNum, elementNum)
    ReDim W(elementNum)
    ...
    DSM(i, j) = Worksheets("DSM").Cells(i + 1, j + 2)
    Next j
    Next i
...
For j = 1 To elementNum + 2
    Worksheets("Banded DSM").Cells(i, j) = Worksheets("DSM").Cells(i, j)
Next j
Next i
...
Do While Sum <> 0
    ...
    Do While (band_complete = False) And (j <= elementNum)            'begin to identify remaining activities in the band
        If W(j) <> 0 Then                                    'if next activity needs work
            For k = i To j - 1
                If (DSM(j, k) <> 0) And (W(k) <> 0) Then    'if dependent on an upstream activity needing work
